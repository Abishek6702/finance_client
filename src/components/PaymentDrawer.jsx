...
const PaymentDrawer = ({ show, onClose, onRefresh, selectedStudent, enteredRows, totalAmount }) => {
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-[28%] bg-white h-[97vh] shadow-xl m-2 rounded-xl flex flex-col">
        ...
        <div className="px-6 py-4 flex items-center justify-between border-b border-gray-200">
          ...
          <div className="bg-gray-100 rounded-full p-1 cursor-pointer hover:bg-gray-200" onClick={onClose}>
        ...
        <div className="flex-1 overflow-y-auto px-6 space-y-4 pt-4">
          ...
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-3">
              <img src={selectedStudent?.img} className="w-11 h-11 rounded-full object-cover border" alt="Profile" />
              <div>
                <p className="font-bold text-gray-800 leading-tight">{selectedStudent?.name}</p>
                <p className="text-xs text-gray-500">{selectedStudent?.year} / {selectedStudent?.dept}</p>
              </div>
            </div>
          ...
          <div className="bg-[#F8F9FA] rounded-xl p-4 border border-gray-100">
            <div onClick={() => setOpen(!open)} className="flex justify-between items-center cursor-pointer">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[#10B981] font-bold text-xl">₹{totalAmount}</span>
              ...
              <div className="mt-3 pt-3 border-t border-gray-200 space-y-2">
                {enteredRows.map((row, i) => (
                  <div key={i} className="flex justify-between text-sm text-gray-600">
                    ...
                    <span className="font-semibold text-gray-700">₹{row.enteredAmount}</span>
                  </div>
                ))}
              </div>
            )}
