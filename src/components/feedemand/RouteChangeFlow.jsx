      ...
      <button
        ...
        className={`w-full flex items-center justify-between border rounded-lg px-3 py-2 text-sm bg-white transition
          ...
          ${disabled ? "bg-gray-50 cursor-not-allowed text-gray-400 border-gray-100" :
            error    ? "border-red-400 focus:ring-red-400 text-gray-800" :
      ...
      {steps.map((label, i) => {
        ...
        return (
          <React.Fragment key={i}>
            ...
            {i < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mb-4 transition-all ${done ? "bg-[#0b56a4]" : "bg-gray-200"}`} />
            )}
          </React.Fragment>
        );
      })}
...
const RouteChangeFlow = ({ student, onClose }) => {
  ...
  // Step 1 calculations
  const balance = Math.max(0, paidAmount - (parseFloat(fee.conceptionAmount) || 0));
  ...
  const needToPay       = newRouteFee !== null ? Math.max(0, newRouteFee - reductionAmt) : null;

  const availableStops = STOPS_MAP[route.newRoute] || [];

  const handleContinue = () => {
    const errors = validateFee(fee);
    if (Object.keys(errors).length > 0) { setFeeErrors(errors); return; }
    setStep(1);
  };

  const handleSubmit = () => {
    const errors = validateRoute(route);
    if (Object.keys(errors).length > 0) { setRouteErrors(errors); return; }
      ...
      {step === 0 && (
        <>
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            ...
            >
              <span className="text-sm font-semibold text-gray-700">Current Route Details</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Transport End Date" required error={feeErrors.endDate}>
              <input
            ...
            <Field label="Conception Amount" required error={feeErrors.conceptionAmount}>
              <input
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <RadioCard
                ...
                onChange={() => setF("refundMode", "wallet")}
              />
            </div>
            {feeErrors.refundMode && (
              <div className="flex items-center gap-1 mt-1.5">
                <AlertCircle className="w-3 h-3 text-red-400 shrink-0" />
                <span className="text-xs text-red-400">{feeErrors.refundMode}</span>
              </div>
            )}
          </div>
        ...
        <>
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Route Stop" required error={routeErrors.newStop}>
              <SearchableSelect
                ...
                placeholder={route.newRoute ? "Select stop" : "Select route first"}
            ...
            <Field label="Effective Date" required error={routeErrors.effectiveDate}>
              <input
          ...
          {routeSelected && (
            <div className="flex flex-col gap-3">
              {/* Total fee read-only */}
              <div className="grid grid-cols-2 gap-3">
                <ReadOnlyField label="Total Route Fee" value={fmt(newRouteFee)} />
