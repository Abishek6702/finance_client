...
const TransportWithdrawalFlow = ({ student, onClose }) => {
  ...
  const totalAmount = transportSummary.total;
  const paidAmount = transportSummary.paid;
  ...
  const fetchTransportData = async () => {
    try {
      ...
      if (json.success) {
        const details = json.data.studentType.transportDetails;
        console.log(
          "transport details:",
          json.data.studentType.transportDetails,
        );
        setTransportSummary({
          total: details?.fee || 0,
          paid: details?.paid || 0,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        refundMode:
          data.refundMode === "wallet"
            ? "wallet"
            : data.refundMethod, // cash OR bank
      ...
      if (res.data.success) {
        toast.success("Transport withdrawal submitted successfully!");
        onClose(); // ✅ only here
      } else {
  ...
  return (
    <div className="flex flex-col gap-6 mt-4">
      ...
      {/* Refund mode only if there is balance */}
      {balance > 0 && (
        <div>
